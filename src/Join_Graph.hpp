...
struct Join_Graph_Node {
  ...
  : evaluated(false), size(0), type(type), aliases(aliases), table(move(t)), left(l), right(r) {
    if (type == Node_Type::CROSSPRODUCT) {
  ...
  shared_ptr<Join_Graph_Node> join(shared_ptr<Join_Graph_Node> node1, shared_ptr<Join_Graph_Node> node2, unique_ptr<Operator> table, Node_Type type=Node_Type::CROSSPRODUCT) {
    ...
    shared_ptr<Join_Graph_Node> new_node (new Join_Graph_Node(move(table), merge_sets<string>(node1->aliases,node2->aliases), type, node1, node2));
    node2->next = new_node;
    node1->next = new_node;
    //new_node->print();
    return new_node;
  ...
  void print(int depth=0) {
    ...
    if (type==Node_Type::SELECT) {
      left->print(depth+1);
    } else if (type==Node_Type::CROSSPRODUCT || type==Node_Type::HASHJOIN) {
      left->print(depth+1);
      right->print(depth+1);
    }
  }
  ...
  string type_str() {
    ...
    if (type==Node_Type::LEAF) {
      ...
      result = "CrossProduct";
    } else if (type==Node_Type::HASHJOIN) {
      result = "HashJoin";
    }
    return result;
  }
